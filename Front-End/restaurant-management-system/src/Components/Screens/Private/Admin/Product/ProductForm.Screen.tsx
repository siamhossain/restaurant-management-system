...
import {Badge, Dialog, IconButton as EvergreenIconButton, Select, Textarea, TextInputField} from "evergreen-ui";
...
interface IProductFormScreenProps {
    formData: IProductViewState['form'],
    gridData: IProductViewState['grid'],

    onFormStateChange(state: { [k in keyof (IProductViewState)['form']]?: (IProductViewState)['form'][k] }, callback?: undefined | (() => void)): void,

    onFormSubmit(): void,

    onFormClose(): void,

    onSKUExistenceCheck(sku: IProduct['sku']): void,
...
const ProductFormScreen: React.FC<IProductFormScreenProps> = (props): ReactElement => {
        ...
        Media: ({uri, index}: { uri?: any, index: number }) => (
            ...
            <span className={css` position: absolute; right: -10px; top: -10px; `}>
                ...
                <CancelIcon style={{fontSize: 18}} onClick={(e: any) => {
                    ...
                    const product_media = props.formData.media;
                    product_media.splice(index, 1);
                        ...
                        <Grid container spacing={3}>
                            <Grid item sm={6} xs={12}>
                                ...
                                <CategoryDropdownScreen
                                    ...
                                    onChange={(uuid, label) => props.onFormStateChange({category_uuid: convertToString(uuid)})}
                                />
                            </Grid>
                            <Grid item sm={6} xs={12}>
                                <div style={{
                                ...
                                <BrandDropdownScreen
                                    width={'100%'}
                                    uuid={convertToString(props.formData.brand_uuid)}
                                    onChange={(uuid) => props.onFormStateChange({brand_uuid: convertToString(uuid)})}/>
                            </Grid>
                            <Grid item sm={6} xs={12}>
                                <TextInputField
                                    label="Product Title"
                            ...
                            <Grid item sm={3} xs={12}>
                                <TextInputField
                                    ...
                                    value={props.formData.sales_price}
                                />
                            </Grid>
                            ...
                            <Grid item sm={3} xs={12}>
                                <TextInputField
                                    ...
                                    value={props.formData.min_stock}
                                />
                            </Grid>
                                ...
                                <FeaturedImagePicker
                                    ...
                                    onPick={(e) => {
                                        if (e.target.files !== null) {
                                            ...
                                            UploaderProvider.upload("product", e.target.files[0], ({data}) => {
                                                props.onFormStateChange({
