...
import {IProduct, ISalesOrder} from "@/App/Interfaces/Models";
...
interface ISalesOrderFormScreenProps {
    formData: ISalesOrderViewState['form'],

    onFormStateChange(state: { [k in keyof (ISalesOrderViewState)['form']]?: (ISalesOrderViewState)['form'][k] }, callback?: undefined | (() => void)): void,
...
const SalesOrderFormScreen:React.FC<ISalesOrderFormScreenProps> = (props): ReactElement => {
    return (
        <Fragment>
            <Dialog
                ...
                <Grid container spacing={1}>
                    ...
                    <Grid item sm={4}>
                        <div className={css`position: relative;`}>
                            <div style={{color: "#101840", fontSize: 14, fontWeight: 500, marginBottom: 5}}>Customer
                            </div>
                    ...
                    <Grid item sm={3}>
                        ...
                        <Select
                            ...
                            onChange={(e: SelectChangeEvent) => {
                                const value: ISalesOrder['payment_type'] = e.target.value === "Credit" ? "Credit" : "Cash";
                        ...
                        <React.Fragment>
                            ...
                            <Grid item sm={2}>
                                <TextInputField
                                    ...
                                    value={props.formData.return_amount.toFixed(2)}
                                />
                            </Grid>
                        </React.Fragment>
                    ) : (
                    ...
                    <Grid item sm={4}>
                        <div className={css`position: relative;`}>
                            ...
                            <ProductDropdownScreen
                                ...
                                onChange={(uuid, stock) => {
                                    if (stock > 0) {
                                        props.onSelectProduct(uuid);
                                    }else {
                                        toaster.danger("This product out of stock!");
                                        return;
                                    }
                                }}
                            />
                    ...
                    <Grid item sm={4}>
                        <div style={{color: "#101840", fontSize: 14, fontWeight: 500, marginBottom: 5}}>Select Sales Type</div>
                        <Select
                            value={props.formData.sales_type}
                            onChange={(event: any) => props.onFormStateChange({sales_type: event.target.value})}
                            width={"100%"}>
                            <option value="Parcel">Parcel</option>
                            <option value="Table_Sale">Table Sale</option>

                        </Select>
                    </Grid>
                    {props.formData.sales_type === 'Table_Sale' && (
                        <Grid item sm={4}>
                            <div style={{color: "#101840", fontSize: 14, fontWeight: 500, marginBottom: 5}}>Select Sales Type</div>
                            <Select
                                value={props.formData.sales_type}
                    ...
                    <Grid item sm={12}>
                        <Table>
                            <Table.Head height={30}>
                                ...
                                <Table.TextHeaderCell>Sales Price</Table.TextHeaderCell>
