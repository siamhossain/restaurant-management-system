...
import {IIngredient, IIngredientPurchase} from "@/App/Interfaces/Models";
import {IIngredientPurchaseViewState} from "@/Views/Private/Admin/IngredientPurchase/IngredientPurchase.View";
import {Dialog, Select, Table, IconButton as EverGreenIconButton, TextInputField, TrashIcon} from "evergreen-ui";
...
interface IIngredientPurchaseFormScreenProps {
    formData: IIngredientPurchaseViewState['form'],

    onFormStateChange(state: { [k in keyof (IIngredientPurchaseViewState)['form']]?: (IIngredientPurchaseViewState)['form'][k] }, callback?: undefined | (() => void)): void,
    ...
    onSelectIngredient(ingredient_uuid: IIngredient['uuid']): void,

    onDeleteParticular(index: number): void,

    onChangeParticularData(data: { index: number, key: 'quantity' | 'unit_price', value: any }, callback?: (() => void) | undefined): void,
}
const IngredientPurchaseFormScreen: React.FC<IIngredientPurchaseFormScreenProps> = (props): ReactElement => {
    return (
        <Fragment>
            <Dialog
                isShown={props.formData.open}
                title="Ingredient Purchase Entry"
                confirmLabel="Save"
                intent={"success"}
                ...
                <Grid container spacing={1}>
                    ...
                    <Grid item sm={3}>
                        ...
                        <Select
                            ...
                            onChange={(e: SelectChangeEvent) => {
                                const value: IIngredientPurchase['payment_type'] = e.target.value === "Credit" ? "Credit" : "Cash";
                    ...
                    {/*<Grid item sm={2}>
                        <TextInputField
                            ...
                            value={props.formData.shipping_cost}
                        />
                    </Grid>*/}
                    ...
                    <Grid item sm={4}>
                        <div className={css`position: relative;`}>
                            <div style={{color: "#101840", fontSize: 14, fontWeight: 500, marginBottom: 5}}>Ingredient
                            </div>
                            <IngredientDropdownScreen
                                width={"100%"}
                                onChange={(uuid) => props.onSelectIngredient(uuid)}
                            />
                        </div>
                    </Grid>
                    ...
                    <Grid item sm={12}>
                        <Table>
                            <Table.Head height={30}>
                                <Table.TextHeaderCell>Ingredient Name</Table.TextHeaderCell>
                            ...
                            <Table.Body height={240}>
                                {props.formData.particulars.map((particular, index) => (
                                    <Table.Row key={index} height={30}>
                                        <Table.TextCell>{particular.ingredient_name}</Table.TextCell>
                                        <Table.TextCell>{particular.ingredient_code}</Table.TextCell>
...
export { IngredientPurchaseFormScreen };
