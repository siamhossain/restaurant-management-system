...
import {IIngredientPurchase} from "@/App/Interfaces/Models";
...
interface IIngredientPurchaseGridScreenProps {
    gridData: IIngredientPurchaseViewState['grid'],
    formData: IIngredientPurchaseViewState['form'],
    onView(data: IIngredientPurchase): any
    onReload(): void,
    onEdit(item: IIngredientPurchase): void,
    onDelete(uuid: string): void
    onNavigateToPageNumber(): void,
    onChangeFilter(state: { [k in keyof (IIngredientPurchaseViewState)['grid']['filter']]?: (IIngredientPurchaseViewState)['grid']['filter'][k] }): void,
    onChangeGridData(state: { [k in keyof (IIngredientPurchaseViewState)['grid']]?: (IIngredientPurchaseViewState)['grid'][k] }): void,
...
const IngredientPurchaseGridScreen: React.FC<IIngredientPurchaseGridScreenProps> = (props): ReactElement => {
    const {filter, ingredient_purchase_orders} = props.gridData
    return (
        <Fragment>
            <DataGrid.Wrapper>
                <DataGrid.FilterBody>
                ...
                <DataGrid.Container>
                    ...
                    <DataGrid.Body>
                        {ingredient_purchase_orders.length > 0 && ingredient_purchase_orders.map((item: IIngredientPurchase, i: number) => (
                            <DataGrid.Row key={i}>
...
export { IngredientPurchaseGridScreen };
