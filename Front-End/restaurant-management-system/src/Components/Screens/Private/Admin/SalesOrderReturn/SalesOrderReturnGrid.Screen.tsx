...
import {ISalesOrder, ISalesOrderReturn} from "@/App/Interfaces/Models";
import {ISalesOrderReturnViewState} from "@/Views/Private/Admin/SalesOrderReturn/SalesOrderReturn.View";
import {DataGrid} from "@/Components/Core/DataGrid";
import {EditIcon, EyeOpenIcon, TrashIcon} from "evergreen-ui";

interface ISalesOrderReturnGridScreenProps {
    gridData: ISalesOrderReturnViewState['grid'],
    formData: ISalesOrderReturnViewState['form'],
    onView(data: ISalesOrderReturn): any
    onReload(): void,
    onEdit(item: ISalesOrderReturn): void,
    onDelete(uuid: string): void
    onNavigateToPageNumber(): void,
    onChangeFilter(state: { [k in keyof (ISalesOrderReturnViewState)['grid']['filter']]?: (ISalesOrderReturnViewState)['grid']['filter'][k] }): void,
    onChangeGridData(state: { [k in keyof (ISalesOrderReturnViewState)['grid']]?: (ISalesOrderReturnViewState)['grid'][k] }): void,
...
const SalesOrderReturnGridScreen: React.FC<ISalesOrderReturnGridScreenProps> = (props): ReactElement => {
    const {filter, sales_order_returns} = props.gridData
    return (
        <Fragment>
            <DataGrid.Wrapper>
                <DataGrid.FilterBody>
                ...
                <DataGrid.Container>
                    ...
                    <DataGrid.Body>
                        {sales_order_returns.length > 0 && sales_order_returns.map((item: ISalesOrderReturn, i: number) => (
                            <DataGrid.Row key={i}>
...
export { SalesOrderReturnGridScreen };
