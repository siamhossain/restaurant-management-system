...
interface ICustomerGridScreenProps {
    open?: boolean,

    onView(data: ICustomer): any

    gridData: ICustomerViewState['grid'],
    formData: ICustomerViewState['form'],

    onEdit(item: ICustomer): void,
    ...
    onChangeFilter(state: { [k in keyof (ICustomerViewState)['grid']['filter']]?: (ICustomerViewState)['grid']['filter'][k] }): void,

    onChangeGridData(state: { [k in keyof (ICustomerViewState)['grid']]?: (ICustomerViewState)['grid'][k] }): void,
...
const CustomerGridScreen: React.FC<ICustomerGridScreenProps> = (props): ReactElement => {
    const {filter, customers} = props.gridData;
                ...
                <DataGrid.Container>
                    <DataGrid.Header>
                        <DataGrid.Row>
                            ...
                            <DataGrid.Heading>Due</DataGrid.Heading>
                    ...
                    <DataGrid.Body>
                        {customers.length > 0 && customers.map((item: ICustomer, i: number) => (
                            <DataGrid.Row key={i}>
                                ...
                                {item.due > item.paid ? (
                                    <DataGrid.Data>{item.due - item.paid}</DataGrid.Data>
                                ): (
                                    <DataGrid.Data>0</DataGrid.Data>
                                )}
