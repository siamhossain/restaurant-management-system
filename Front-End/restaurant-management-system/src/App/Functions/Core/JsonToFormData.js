/**
 * Convert the json data into form data
 */
...
function convert(jsonObject, options = {
    ...
    if (options && options.initialFormData) {
        ...
        if (!isAppendFunctionPresent(options.initialFormData)) {
            ...
            throw new Error('initialFormData must have an append function.');
        }
    } else if (!isGlobalFormDataPresent()) {

        throw new Error('This environment does not have global form data. options.initialFormData must be specified.');
    }
