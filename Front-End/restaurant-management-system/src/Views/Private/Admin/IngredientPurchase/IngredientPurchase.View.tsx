...
import {
    ...
    IngredientPurchaseGridScreen, IngredientPurchaseViewScreen
} from "@/Components/Screens/Private/Admin/IngredientPurchase";
import {convertToString} from "@/App/Functions/Custom";
import {toaster} from "evergreen-ui";
import {IngredientPurchaseProvider, SocialLinkProvider} from "@/App/Services/Providers/Modules/Admin";
import {convertToNumber} from "@/App/Functions/Custom";
import {parseDate} from "@/App/Functions/Custom/parseDate.Function";

export interface IIngredientPurchaseViewProps {

}

export interface IIngredientPurchaseViewState {
    grid: {
        dialog_open: boolean,
        ...
        ingredient_purchase_orders: IIngredientPurchase[],
        filter: {
            search: string,
        }
    }
    ...
    form: {
        ...
        open: boolean,
        uuid: string,
        supplier_uuid: IIngredientPurchase['supplier_uuid'],
        date: IIngredientPurchase['date'] | Date,
        payment_type: IIngredientPurchase['payment_type'],
        total_amount: IIngredientPurchase['total_amount'] | any,
        discount: IIngredientPurchase['discount'] | any,
        vat: IIngredientPurchase['vat'] | any,
        tax: IIngredientPurchase['tax'] | any,
        shipping_cost: IIngredientPurchase['shipping_cost'] | any,
        payable_amount: IIngredientPurchase['payable_amount'] | any,
        received_amount: IIngredientPurchase['received_amount'] | any,
        return_amount: IIngredientPurchase['return_amount'] | any,
        paid_amount: IIngredientPurchase['paid_amount'] | any,
        due_amount: IIngredientPurchase['due_amount'] | any,
    ...
    view: {
        open: boolean,
        data: Partial<IIngredientPurchase>
    }

    additional: {}
}
class IngredientPurchaseView extends View<IIngredientPurchaseViewProps, IIngredientPurchaseViewState> {
    ...
    private getPurchaseOrders(): void {
        IngredientPurchaseProvider.getIngredientPurchasesForGrid(
            ...
            (data) => {
                ...
                this.setGridState({ingredient_purchase_orders: data.ListData});
            });
    }
    ...
    private generateAndGetParticulars(ingredient_uuid: IIngredient['uuid']): void {
        const particulars = [...this.state.form.particulars];

        const particularIndex = particulars.findIndex(particular => convertToString(particular.ingredient_uuid) === convertToString(ingredient_uuid));
        if (particularIndex > -1) {
            toaster.danger("This product has been taken!");
            return;
        }

        IngredientPurchaseProvider.generateAndGetParticulars(ingredient_uuid, (data) => {
            this.setFormState({
    ...
    private savePurchaseOrder(): void {
        ...
        IngredientPurchaseProvider.saveIngredientPurchase(this.state.form, (data) => {
                    ...
                    <div className="wrapper" style={{display: "flex", flexDirection: "column", flexGrow: 1}}>
                        ...
                        <div className={MainContainerStyleSheet.classes.root}>
                            <div className={"main-container"} style={{paddingTop: '10px'}}>
                                <ModuleTitleScreen
                                    moduleTitle={"Ingredient Purchase List"}
                                ...
                                <IngredientPurchaseGridScreen
                                    ...
                                    onDelete={(uuid) => {
                                        if (window.confirm("Are you sure?")) {
                                            IngredientPurchaseProvider.deleteIngredientPurchase(uuid, (data) => {
                                                toaster.success(data.message);
                                                this.getPurchaseOrders();
                                            })
                                        }
...
export default IngredientPurchaseView;
