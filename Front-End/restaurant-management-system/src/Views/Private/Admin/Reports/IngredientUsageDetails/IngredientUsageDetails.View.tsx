...
import {IngredientUsesProvider, PurchaseOrderProvider, UserProvider} from "@/App/Services/Providers/Modules/Admin";
...
class IngredientUsageDetailsView extends View<any, any> {
    ...
    private getAllOrderList() {
        IngredientUsesProvider.getIngredientsUsesNoLimit('', (data) => {
            this.setGridState({uses_list: data});
        })
    }
                    ...
                    <div className="wrapper" style={{display: "flex", flexDirection: "column", flexGrow: 1}}>
                        ...
                        <div className={MainContainerStyleSheet.classes.root}>
                            <div className={"main-container"} style={{paddingTop: '10px'}}>
                                <ModuleTitleScreen
                                    moduleTitle={"Ingredient Usage Details"}
                                />
                                <Grid container spacing={2} style={{marginLeft: "25px"}}>
                                    <Grid item lg={3}>
                                        ...
                                        >
                                            ...
                                            {this.state.grid.uses_list.map((item: any, index: number) => (
                                    ...
                                    <Grid item lg={3}>
                                        <div style={{color: "#101840", fontSize: 14, fontWeight: 500, marginTop: 5}}>
                                            <Button onClick={() => window.open(ApiConfig.REPORTS_ROOT + "/ingredient-uses-details?uuid=" + this.state.form.purchase_uuid)} marginLeft={16} marginTop={20} appearance="primary" intent="success">
                                                Submit
                                            </Button>
                                        </div>
